...
class CustomerController extends Controller
{
    ...
    {
        if (! auth()->guard('customer-pi')->attempt(request(['email', 'password']))) {
            return responder()->error(  401,"Invalid data")->respond();
        }

        if (auth()->guard('customer-api')->user()->status == 0) {
            auth()->guard('customer-api')->logout();
            return responder()->error( 203,"Customer not acctivated" )->respond();
        }

        if (auth()->guard('customer-api')->user()->is_verified == 0) {
