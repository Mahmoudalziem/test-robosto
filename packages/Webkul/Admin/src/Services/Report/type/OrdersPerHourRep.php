...
class OrdersPerHourRep {

    public $name;
    protected $data;

    public function __construct(array $data) {
        $this->name = "orders-per-hour";
        $this->data = $data;
    }
    ...
    public function getMappedQuery() {
        ...
        $select = " SELECT 
                        COUNT(id) orders_count,
                        DATE_FORMAT(CONCAT(DATE(created_at), ' ', HOUR(created_at)),'%H') AS 'hours'
                    FROM (SELECT distinct orders.id AS id, area_id, orders.created_at,status,orders.shippment_id
                           FROM orders
                    ...
                    $area
                    $notShippment
                    GROUP BY DATE_FORMAT(CONCAT(DATE(created_at), ' ', HOUR(created_at)),
                    '%H')
                    ORDER BY hours ";
    ...
    public function getHeaddings() {
        return ['hours', 'orders_count', 'hours'];
    }
